...
const getVisibleLinks = (links, maxItems) => {
	...
	return {
		...
		visible: shouldShowMore
			? links.slice(0, maxItems)
			: links,
		hidden: shouldShowMore
			? links.slice(maxItems)
			: [],
	};
};
...
export function FloatingDockClient({ navItems }) {
	...
	// Map Sanity items directly to links
	...
	const desktop = getVisibleLinks(
		links,
		MAX_VISIBLE_ITEMS_DESKTOP,
	);
	const mobile = getVisibleLinks(
		links,
	...
	return (
		<>
			{/* Desktop: Horizontal dock - Fixed position (Bottom Center) */}
			<div className="hidden md:block fixed z-100 bottom-4 left-1/2 -translate-x-1/2 transition-all duration-300 pointer-events-none group/dock">
				<div className="flex items-center gap-2 px-3 py-2.5 rounded-2xl bg-white/20 dark:bg-black/30 hover:bg-white/30 dark:hover:bg-black/40 backdrop-blur-xl border border-white/30 dark:border-white/20 hover:border-white/40 dark:hover:border-white/30 shadow-[0_8px_32px_0_rgba(0,0,0,0.15)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] pointer-events-auto transition-all duration-300">
					{/* Render Visible Desktop Items */}
						...
						<div className="relative">
							<button
								...
								<div className="relative flex items-center justify-center w-full h-full rounded-full bg-white/10 dark:bg-white/5 group-hover/dock:bg-white/40 dark:group-hover/dock:bg-white/20 backdrop-blur-md border border-white/20 dark:border-white/10 group-hover/dock:border-white/50 dark:group-hover/dock:border-white/30 transition-all duration-500 ease-out hover:scale-125 hover:-translate-y-3 hover:bg-white/50 dark:hover:bg-white/30 hover:border-white/70 dark:hover:border-white/40 hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)]">
									<div className="w-6 h-6 text-neutral-400/60 group-hover/dock:text-neutral-500 dark:text-neutral-300/60 dark:group-hover/dock:text-neutral-300 group-hover:text-neutral-600 dark:group-hover:text-neutral-200 transition-colors duration-300">
										{desktopMoreMenuOpen ? (
							...
							{desktopMoreMenuOpen && (
								<div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-50 flex flex-col-reverse gap-2 p-3 rounded-xl bg-white/90 dark:bg-black/90 backdrop-blur-xl border border-white/40 dark:border-white/30 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.6)] animate-in slide-in-from-bottom-2 duration-200">
			...
			<div className="md:hidden fixed top-4 right-4 z-100">
				...
				{mobileMenuOpen && (
					<div className="absolute top-14 right-0 z-50 flex flex-col gap-2 p-3 rounded-xl bg-white/90 dark:bg-black/90 backdrop-blur-xl border border-white/40 dark:border-white/30 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.6)] animate-in slide-in-from-top-2 duration-200">
						...
						{mobile.shouldShowMore && (
							<div className="relative">
								...
								{/* Mobile More Menu Expanded */}
								{mobileMoreMenuOpen && (
									<div className="absolute top-0 right-16 z-50 flex flex-row-reverse gap-2 p-3 rounded-xl bg-white/90 dark:bg-black/90 backdrop-blur-xl border border-white/40 dark:border-white/30 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.6)] animate-in slide-in-from-right-2 duration-200">
...
// Icon Component (Cleaned up logic)
function DockIcon({
	item,
	isVertical,
	onItemClick,
		...
		return (
			<div
				className={`absolute px-3 py-1.5 ${isHorizontal ? "rounded-xl" : "rounded-lg"} bg-white/90 dark:bg-black/90 backdrop-blur-xl border border-white/40 dark:border-white/20 ${isHorizontal ? "text-xs md:text-sm" : "text-sm"} font-medium text-neutral-800 dark:text-neutral-200 whitespace-nowrap opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 pointer-events-none shadow-[0_8px_32px_0_rgba(0,0,0,0.2)] ${
					isHorizontal
						? "-top-12 left-1/2 -translate-x-1/2 group-hover:-translate-y-2"
		...
		<>
			<div
				className={
					isVertical
						? verticalIconClasses
						: horizontalIconClasses
				}>
				<div
					className={`w-6 h-6 ${isVertical ? "text-neutral-500 dark:text-neutral-300" : "text-neutral-400/60 group-hover/dock:text-neutral-500 dark:text-neutral-300/60 dark:group-hover/dock:text-neutral-300 group-hover:text-neutral-600 dark:group-hover:text-neutral-200 transition-colors duration-300"}`}>
					{item.icon}
				</div>
			</div>
			<Tooltip
